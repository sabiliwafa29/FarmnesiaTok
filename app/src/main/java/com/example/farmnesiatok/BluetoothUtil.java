...
public class BluetoothUtil {
    ...
    private static void showSettingsDialog(Fragment fragment) {
        ...
        builder.setNegativeButton("Cancel", null);
        builder.setPositiveButton("Settings", (dialog, which) -> fragment.startActivity(new Intent(android.provider.Settings.ACTION_APPLICATION_DETAILS_SETTINGS, Uri.parse("package:" + BuildConfig.APPLICATION_ID))));
        builder.show();
    }
    ...
    static boolean hasPermissions(Fragment fragment, ActivityResultLauncher<String[]> requestPermissionLauncher) {
        if(Build.VERSION.SDK_INT < Build.VERSION_CODES.S)
            return true;
        boolean missingPermissions = fragment.getActivity().checkSelfPermission(android.Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED
                | fragment.getActivity().checkSelfPermission(android.Manifest.permission.BLUETOOTH_SCAN) != PackageManager.PERMISSION_GRANTED;
        boolean showRationale = fragment.shouldShowRequestPermissionRationale(android.Manifest.permission.BLUETOOTH_CONNECT)
                | fragment.shouldShowRequestPermissionRationale(android.Manifest.permission.BLUETOOTH_SCAN);
